...
- Verified the npm artifact rather than only the repo checkout: `data/gateway-policy.json` is in the tarball, and installing the packed tarball into a fresh project resolves `DEFAULT_GATEWAY_POLICY_PATH` inside the installed package and reads the shipped file. This matters because the fail-closed policy branch means a packaging defect would otherwise refuse every invocation instead of degrading to the built-in default.
...
### Gateway hardening — the Eigendark requirements on the invoke path (2026-08-16)

Applies the hardening list recorded in `TODO.md` (P1, 2026-08-15) to `controlcenter_invoke`. That entry was written as a policy to *write down* for future remote-MCP work; it is treated here as a requirements list, because the gateway is exactly the surface it describes. New module `src/gatewayHardening.ts`, kept separate so the limits are testable on their own.

- Redact forwarded results recursively, at every nesting level, before they reach the caller's context. Two signals with deliberately different reach: high-specificity credential shapes inside free text (`sk-…`, `ghp_…`, `github_pat_…`, `AKIA…`, `xox[baprs]-…`, JWTs, `AIza…`) are replaced **everywhere**, while object keys that name a secret (`token`, `apiKey`, `password`, `cookie`, `private_key`, …) have their value replaced whole **only inside `structuredContent`**.
- Keep the key rule off content blocks, because those carry what the caller asked to read. Measured on the flagship path: a config file returned as a text block was already safe (its contents are a string, so only the narrow patterns apply), but an object-shaped content block lost the value of a field called `auth` — handing back a different file than the one requested, without saying so. A real token inside a content block is still redacted; a requested payload is not.
- Deliberately keep the value patterns narrow. Result payloads carry real user data, and a loose `key=value` mask would silently corrupt a config file or source listing that a caller asked to read. The aggressive masking stays where it was — on error strings, where corruption costs nothing. The result reports how many values were replaced, so a modified payload is never silently modified.
- Add `redactResults` to the policy file for callers that need byte-exact payloads. Default `true`; turning it off is a documented, deliberate weakening rather than a hidden switch.
- Add finite budgets, all overridable by environment variable: request bytes (`ELLMOS_GATEWAY_MAX_REQUEST_BYTES`, 256 KiB), response bytes (`ELLMOS_GATEWAY_MAX_RESPONSE_BYTES`, 1 MiB), nesting depth (`ELLMOS_GATEWAY_MAX_DEPTH`, 32), content blocks (`ELLMOS_GATEWAY_MAX_CONTENT_BLOCKS`, 200), and concurrent invocations (`ELLMOS_GATEWAY_MAX_CONCURRENT`, 4).
- Treat the request and the response budgets differently, on purpose. Oversized **arguments are refused**, never shortened: a truncated argument set would silently change what the caller asked the target to do. An oversized **response is truncated and flagged**, so the part that arrived is still usable — structured content is dropped first because most servers duplicate it in the text blocks, then trailing blocks, then the last block is cut with a visible marker.
- Bound concurrency with a shared limiter. Without it, a batch of parallel tool calls would spawn one backend process each, at the same time — the exact resource class that connect-per-call was chosen to avoid. A call that cannot get a slot within its own timeout is refused with `budget-exceeded` rather than queueing without end.
- Make the recursive walk safe against a hostile or looping backend: bounded by the depth budget and cycle-detecting, so a self-referential payload cuts off with a marker instead of recursing forever. An unserializable payload counts as over budget rather than being waved through.
- Enforce a transport policy on remote targets: HTTPS everywhere, plain HTTP only on loopback where there is no network to eavesdrop on, other schemes refused. An optional `allowedRemoteHosts` list (with `*.` subdomain wildcards) narrows this further; empty means any HTTPS host, since the profile already bounds which servers exist at all.
- Refuse HTTP redirects on gateway calls (`redirect: "error"`). A redirect could move an authenticated MCP request to a host that never passed the target policy. `createTransport` gained an optional hardening parameter for this; the tool-scanning path is unchanged.
- Mark forwarded payloads as untrusted data in the text output, with a banner before the content and a closing marker after it. The gateway pipes third-party server output into a calling agent's context, so that output must read as data, not as instructions.
- Extend the audit entry with `responseBytes`, `truncated`, and `redactions`. Sizes and counts only — result content still never reaches the log.
- Add two outcomes so a refusal is never mistaken for a target failure: `budget-exceeded` and `transport-denied`.
- Add `test/gatewayHardening.test.ts` (25 tests) for redaction, depth and cycle limits, both budget directions, the URL policy, and the limiter; plus 9 integration tests in `test/gateway.test.ts` proving each control fires on the real invoke path — including a deterministic concurrency proof where a holder keeps the only slot for 4 s while two waiters give up after 700 ms. Full suite: 202 tests.
- Still open, and listed in `TODO.md`: opaque session-bound capabilities have no counterpart here, because the gateway holds no session and issues no capability handle; per-risk-class policies from backend tool annotations; and budget enforcement on the tool-scanning path, which currently only inherits the URL policy through shared code, not the response limits.
