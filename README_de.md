...
[Installation](#installation) • [Zielgruppen & Auffindbarkeit](#zielgruppen--auffindbarkeit) • [Vergleichsmatrix](#vergleichsmatrix-gegenueber-alternativen) • [Systemarchitektur](#systemarchitektur) • [Control- & Gateway-Ablauf](#control-plane--gateway-lebenszyklus) • [Governance-Invarianten](#governance--und-laufzeit-invarianten) • [Status](#status) • [Tools (34)](#tools) • [Gateway](#gateway-server-erreichen-die-der-host-nicht-geladen-hat) • [Capability-Bundles](#capability-bundles) • [Profilwechsel](#profilwechsel) • [Host-Register](#host-register-sperren-rechte-entscheidungen-governance-ressourcen) • [Dashboard](#dashboard) • [Drittanbieter-Lizenzen](#drittanbieter-lizenzen--transparenz) • [Dokumentation](#dokumentation) • [Sicherheitsrichtlinie](SECURITY.md) • [llms.txt Kontext](llms.txt) • [Ökosystem-Matrix](#ellmos-ai-ökosystem)
...
| **Profil- & Stack-Verwaltung** | **Extends-Ketten, dynamische Bündel & Katalog-Probes** | Manuelle JSON-Pflege | Fest kodierte Server-Arrays | Programmatische Python-Klassen | Web-Dashboard-Konfiguration |
| **Internationalisierung (i18n)** | **Zweisprachiger Kern (EN/DE Laufzeit-Ausgabe)** | Nur Englisch | Nur Englisch | Nur Englisch | Nur Englisch |
| **Sicherheits-SLA & Supply-Chain** | **48h SLA, 0 Telemetrie-Abhängigkeiten, auditierte Lizenzen** | Herstellerabhängig | Ungeprüfte Drittanbieter-Tools | Große Angriffsfläche (100+ pip-Pakete) | Vertrauen in externe Cloud-Provider |
...
sequenceDiagram
    ...
    participant Res as Profil- & Capability-Resolver
    participant Gate as Gateway-Policy-Guard
    participant Backend as Backend MCP Server (Ungeladen)
    participant Scrub as Härtung & Secret-Scrubber
    participant Audit as Audit-Logger (JSONL)

    Note over Agent,CC: 1. Administration & Profil-Discovery
    Agent->>CC: controlcenter_suggest_profile / resolve_profile
    CC->>Res: Prüft ~/.claude/profiles & extends-Ketten
    Res-->>CC: Aufgelöste MCP-Konfiguration & Bundles
    CC-->>Agent: Empfohlenes Profil & --mcp-config Flags

    Note over Agent,CC: 2. Richtliniengesteuerte Gateway-Ausführung
    Agent->>CC: controlcenter_invoke(server, tool, args)
    CC->>Gate: Prüft data/gateway-policy.json
    alt Richtlinie verweigert oder fehlerhaft
        Gate-->>CC: Verweigerung (Fail-Closed)
        CC->>Audit: Protokolliert Ablehnung (nur Namen, 0 Werte)
        CC-->>Agent: Fehler: Tool / Server durch Richtlinie blockiert
    else Richtlinie erlaubt
        Gate-->>CC: Aufruf freigegeben
        CC->>Backend: Connect-per-Call (stdio / Streamable HTTP)
        Backend-->>CC: Rohantwort des Backend-Tools
        CC->>Backend: Prozess beenden / Transport schließen
        CC->>Scrub: Rekursive Secret-Schwärzung & Budgetprüfung
        Scrub-->>CC: Bereinigte Nutzlast & Kürzungsstatus
        CC->>Audit: Strukturiertes Audit-Event (gateway-audit.jsonl)
        CC-->>Agent: Sicheres Tool-Ergebnis mit Daten-Bannern
    end
```
...
| `controlcenter_list_locks` | Listet aktive `LOCK*.txt`-Projektsperren über die konfigurierten Wurzeln — siehe [Host-Register](#host-register-sperren-rechte-entscheidungen-governance-ressourcen) |
...
`resources.inventory` des ControlRoom-Programms, nicht bei diesem Server — dieser Spiegel kann
zwischen zwei Syncs veralten und behauptet nie das Gegenteil.

Sie sind **rein lesend**. Keine Sperre wird gesetzt, erneuert oder gelöst, keine Entscheidung
beantwortet. Insbesondere `LOCK.user.*` entfernt ausschließlich der Nutzer; nichts hier kann sie
anfassen.

**Sie fallen zur sicheren Seite.** Ist ein Register nicht konfiguriert, ein Pfad unlesbar, der
Interpreter nicht vorhanden oder die Prüfung fehlerhaft, lautet das Urteil `unknown` und
*safe to proceed* ist `no` — niemals ein beruhigendes „frei". Ein Sperrprüfer, der im Zweifel in
die beruhigende Richtung rät, ist gefährlicher als gar keiner.

**Vererbung wird berücksichtigt.** Eine `LOCK.txt` in einem Elternordner sperrt alles darunter;
`controlcenter_check_lock` läuft deshalb die gesamte Elternkette hoch und meldet die *wirksame*
Sperre samt Abstand — nicht bloß eine Datei im selben Ordner.

**Die Sperr-Semantik wird hier nicht nachgebaut.** Ein kleines Brückenskript delegiert jede Regel
— Verfall, geschützte Sperrtypen, Scope-Zerlegung, Rechte-Präzedenz `deny > ask > allow > default`
— an die kanonischen Python-Module des Hosts. Eine zweite Implementierung würde bei der nächsten
Änderung der Spezifikation auseinanderlaufen. Dies ist die einzige Stelle, an der der Server
Python aufruft; fehlt ein Interpreter, fallen die Werkzeuge zur sicheren Seite wie bei jeder
anderen unerfüllten Voraussetzung.

### Konfiguration

Die Werkzeuge sind **ohne Konfiguration still inaktiv** — auf einer Maschine, die diese Register
nicht eingerichtet hat, gibt es sie schlicht nicht:

| Variable | Zweck |
|---|---|
| `ELLMOS_LOCK_SCRIPTS` | Verzeichnis mit den kanonischen `lock_utils.py`, `permissions.py` und `lock_scan.py`. Pflicht für die drei Sperr- und Rechte-Werkzeuge. |
| `ELLMOS_LOCK_ROOTS` | Optionaler Pfad zu `lock_roots.json`. Standard ist die Datei neben den Lock-Skripten. |
| `ELLMOS_DECISIONS_ROOT` | Verzeichnis mit der Entscheidungskette und ihrem generierten Index. Pflicht für `controlcenter_list_decisions`. |
...
`controlcenter_list_decisions` liefert Kennung, Datum, Titel, Status und Scope — **nicht** den
Fragetext, die Optionen oder die Empfehlung, die persönliche Umstände enthalten können. Die
...
`controlcenter_list_locks` läuft über jede konfigurierte Wurzel. Über cloud-synchronisierten
Speicher dauert das Minuten, deshalb läuft der Scan unter einem Zeitbudget, das **zwischen** den
Wurzeln geprüft wird. Reicht es nicht, wird das Ergebnis als **unvollständig** markiert und nennt
die Wurzeln, die nie erreicht wurden — ein unvollständiger Scan beweist über sie nichts. Für einen
einzelnen Pfad ist `controlcenter_check_lock` das richtige Werkzeug und antwortet in
Millisekunden.
...
## Such- und Discovery-Kontext

Für Links und Suchen ist der volle Name **ellmos ControlCenter MCP** oder der Paketname `ellmos-controlcenter-mcp` am eindeutigsten. Die kurze Phrase „Control Center" ist zu breit, und „ellmos" kollidiert in Suchmaschinen mit Elmo-/ELMO-Treffern aus Motion-Control, HR-Software und Voice-Generatoren.
...
| [workflowhooker-provenance](https://github.com/ellmos-ai/workflowhooker-provenance) | Local-first Lifecycle-Hooks, Scope-Leitplanken & Laufzeit-Provenienz |
...
| Projekt | Ökosystem | Fokus |
|---|---|---|
| [build-your-users-mind](https://github.com/research-line/build-your-users-mind) | `research-line` | Open-Science Rekonstruktion mentaler Nutzermodelle & Kognitionsframework |

### Desktop-Software & Begleitwerkzeuge

Unsere Partnerorganisation **[open-bricks](https://github.com/open-bricks)** bündelt KI-native Desktop-Anwendungen — eine moderne Open-Source-Softwaresuite für das KI-Zeitalter.

| Projekt | Ökosystem | Fokus |
|---|---|---|
| [ProFiler](https://github.com/open-bricks/ProFiler) | `open-bricks` / `file-bricks` | Erweiterte Dateiverwaltung, Prüfsummen, Duplikatserkennung und Batch-Operationen |
| [lock-master](https://github.com/file-bricks/lock-master) | `open-bricks` / `file-bricks` | Local-first Multi-Agenten-Projektsperren und Rechte-Governance |
| [DokuZen](https://github.com/open-bricks/DokuZen) | `open-bricks` / `doc-bricks` | Dokumentenverwaltung, Textextraktion, OCR und PDF-Verarbeitung |
| [UniversalDocsGrabber](https://github.com/doc-bricks/UniversalDocsGrabber) | `open-bricks` / `doc-bricks` | Universelle Dokumenten-Erfassung, Batch-Ingestion, OCR & Textnormalisierung |
| [safe-start-for-codex](https://github.com/dev-bricks/safe-start-for-codex) | `open-bricks` / `dev-bricks` | Lokaler Laufzeit-Wächter und Umgebungsvalidator für KI-Coding-Agenten |
| [automation-master](https://github.com/dev-bricks/automation-master) | `open-bricks` / `dev-bricks` | Multi-Agenten-Koordination und Hintergrund-Automationsengine |
